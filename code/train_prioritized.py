...
from network import ReplayBuffer, ReplayBufferGPU_Prio, DQN, DQN_Conv, AVAILABLE_DEVICE
...
replay_buffer_prio = ReplayBufferGPU_Prio(replay_buffer_capacity, env.observation_space.shape)
s = env.reset()
...
if(args.load_networks):
    ...
    for i in range(replay_init_size):
        ...
        replay_buffer_prio.add([s,a,s1,r,done])
        s = s1
        if(done):
            s = env.reset()
            done = False
